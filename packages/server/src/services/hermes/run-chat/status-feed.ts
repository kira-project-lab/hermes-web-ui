...
type SessionStateWithStatus = SessionState & {
  pendingApproval?: {
    approval_id: string
    choices?: unknown
...
export function sessionRuntimeStatus(sessionId: string, state: SessionStateWithStatus) {
...
export function sessionRuntimeStatusSnapshot(profile: string, sessionMap: Map<string, SessionStateWithStatus>) {
  const sessions = []
